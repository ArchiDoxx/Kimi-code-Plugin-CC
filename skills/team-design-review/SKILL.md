...
# team-design-review — external module / interface design review

Use this for a **single focused review** of a proposed module boundary or
interface design (a "team"). An external agent assesses coupling direction,
...
`contract-audit` checks an existing team against frozen wire, `team-design-
...
- You are refactoring a team (moving a boundary, splitting a module).
...
**Read the relevant code first** (the modules the team touches, plus any
...
CONTEXT (what exists today, the modules/layering the team touches):
---
<paste the relevant current code, module map, or dependency layout>
---

PROPOSED TEAM (the boundary or interface under review):
---
<paste the proposed interface: signatures, types, which module depends on
which, what the team hides vs exposes, or an ASCII diagram of the layers>
---
Stated intent of the team: <one or two sentences — what decision it should
hide, what it should let vary independently>

Review and report, grouped by issue TYPE. Each finding carries its own SEVERITY
so the team-breaking problems surface first.
...
Issue types:
  ...
  - COUPLING   — two modules joined by more than the stated team (shared
                 mutable state, type leakage, implicit ordering).
  - MISSING    — a responsibility with no home, or a capability the team blocks.
  - TESTABILITY— the team as designed is hard to test in isolation.

Severity (per finding):
  - BLOCKER — the team as designed cannot hold (wrong dependency direction,
              a cycle, or the hidden decision leaks out).
  - MAJOR   — a real coupling/leak that hurts, but the team is salvageable.
  - MINOR   — a small leak or testability friction, low impact.

For each finding output: Type, Severity, Location (which side of the team),
...
DIRECTION or LEAK) — these break the team — and quote the concrete fixes.
...
mcp__kimi-code-plugin-cc__run_agent(
  agent_name="kimi",
  prompt="<filled design-review brief for the new assessment/storage team>"
)
```
